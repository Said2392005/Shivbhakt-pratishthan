...
export default function Navbar() {
  ...
  return (
    <>
      <header className="fixed top-0 left-0 right-0 z-50" style={{ background: "#E67E22" }}>
        <div className="container-site h-[68px] flex items-center justify-between">
          ...
          <a href="#home" className="flex items-center gap-3 flex-shrink-0">
            <img
              ...
              style={{
                filter:
                  "drop-shadow(0 0 1.5px #fff) drop-shadow(0 0 1.5px #fff) drop-shadow(0 0 1.5px #fff) drop-shadow(0 2px 6px rgba(0,0,0,0.45))",
              }}
            />
            <div className="hidden sm:block">
              <p className="font-devanagari font-semibold text-sm text-white leading-tight">शिवभक्त प्रतिष्ठाण</p>
              <p className="text-[10px] font-medium tracking-widest text-white/60 uppercase">Maharashtra State</p>
            </div>
          </a>
          ...
          <nav className="hidden lg:flex items-center gap-6">
            {NAV.map((item) =>
              item.children ? (
                ...
                >
                  <button className="flex items-center gap-1 px-3 py-2 text-sm font-medium text-white/85 hover:text-white transition-colors duration-200">
                    {item.label}
            ...
            >
              <div className="flex items-center justify-between px-5 py-4 border-b border-white/20">
                <div className="flex items-center gap-2.5">
                  ...
                  />
                  <span className="font-devanagari text-sm font-semibold text-white">शिवभक्त प्रतिष्ठाण</span>
                </div>
                  ...
                  item.children ? (
                    <div key={item.label}>
                      <button
                        ...
                        className="w-full flex items-center justify-between px-4 py-3 rounded text-white/85 hover:text-white hover:bg-white/10 font-medium text-sm transition-colors"
                      >
                        {item.label}
                        <ChevronDown
                          size={14}
                          ...
                          >
                            {item.children.map((child) => (
                              <a
                                ...
                                href={child.href}
                                onClick={() => setOpen(false)}
                                className="block px-4 py-2.5 text-sm text-white/70 hover:text-white hover:bg-white/10 rounded transition-colors"
                              >
                                {child.label}
                              </a>
                            ))}
                    ...
                    <motion.a
                      ...
                      className="flex items-center px-4 py-3 rounded text-white/85 hover:text-white hover:bg-white/10 font-medium text-sm transition-colors"
                    >
                      {item.label}
                    </motion.a>
                  )
